...
for (fo in alllfolders) {
  ...
  i = i + 1
  print("2mg")
  data2mg = read.csv(sprintf(
    "%s/All_CADo1PKReview_naloxone_formulation_Generic_conc_%s.csv",
    mainfol,
    fo
  )) # CA Population Data
  Generic_1d = read.csv(sprintf(
    "%s/Im_plot_Generic/%s_ypred1.csv",
    mainfol2,
    dose[i]
  )) # Optimal Patient Physiology data No naloxone
  Generic_2d = read.csv(sprintf(
    "%s/Im_plot_Generic/%s_ypred2.csv",
    mainfol2,
    dose[i]
  )) # Optimal Patient Physiology data	One dose naloxone

  data2mg$Ndose = c("No naloxone", "2mg/2ml")

  print("EVZIO")

  dataEvzio2mg = read.csv(sprintf(
    "%s/All_CADo1PKReview_naloxone_formulation_EVZIO_conc_%s.csv",
    mainfol,
    fo
  )) # CA Population Data
  EVZIO_1d = read.csv(sprintf(
    "%s/Im_plot_EVZIO/%s_ypred1.csv",
    mainfol2,
    dose[i]
  )) #Optimal Patient Physiology data No naloxone
  EVZIO_2d = read.csv(sprintf(
    "%s/Im_plot_EVZIO/%s_ypred2.csv",
    mainfol2,
    dose[i]
  )) #Optimal Patient Physiology data One dose naloxone
  ...
  if (min(Generic_1d$Blood) <= CABloodFlow) {
    ...
    timeIndexUL_G1 <- which(
      abs(Generic_1d[, "time"] - (Generic_1d[timeIndexULL_G1, "time"])) ==
  ...
  if (min(Generic_2d$Blood) <= CABloodFlow) {
    ...
    timeIndexUL_G2 <- which(
      abs(Generic_2d[, "time"] - (Generic_2d[timeIndexULL_G2, "time"])) ==
  ...
  if (min(EVZIO_2d$Blood) <= CABloodFlow) {
    ...
    timeIndexUL_E2 <- which(
      abs(EVZIO_2d[, "time"] - (EVZIO_2d[timeIndexULL_E2, "time"])) ==
  ...
  both$Ndose <- factor(
    both$Ndose,
    levels = c("No naloxone", "2mg/2ml", "2mg/0.4ml")
  )
  ...
  p[[i]] <- ggplot(
    both,
    aes(x = as.factor(Ndose), y = 100 - rowYesCA, fill = type)
  ) +
    ...
    scale_fill_manual(
      name = "Naloxone Dose",
      values = c(a = colorPalette[1], b = colorPalette[2], c = colorPalette[3]),
      labels = c("No naloxone", "IM 2 mg/2 mL", "IM 2 mg/0.4 mL")
    ) +

    theme(legend.position = "none", axis.text = element_text(size = 8))
  ggsave(
    sprintf("results/CA_individual_%s.png", fo),
    p[[i]],
    width = 6,
    height = 4.5
  )
  ...
  p2[[i]] <- ggplot(All_time, aes(x = time / 60, y = Vent, color = type)) +
    geom_line(
      data = Generic_1d[1:timeIndexUL_G1, ],
      aes(x = time / 60, y = Vent)
    ) +
    geom_line(
      data = Generic_2d[1:timeIndexUL_G2, ],
      aes(x = time / 60, y = Vent)
    ) +
    geom_line(
      data = EVZIO_2d[1:timeIndexUL_E2, ],
      aes(x = time / 60, y = Vent)
    ) +
    geom_point(
      data = Generic_1d[timeIndexUL_G1[1], ],
      aes(x = time / 60, y = Vent),
      shape = 4,
      color = "red",
      size = 5
    ...
    geom_point(
      data = Generic_2d[timeIndexUL_G2[1], ],
      aes(x = time / 60, y = Vent),
      shape = 4,
      color = "red",
      size = 5
    ...
    geom_point(
      data = EVZIO_2d[timeIndexUL_E2[1], ],
      aes(x = time / 60, y = Vent),
      shape = 4,
      color = "red",
      size = 5
    ...
    xlim(0, 15)
  ggsave(
    sprintf("results/plot_Ventilation_Individual_%s.png", fo),
    p2[[i]],
    width = 6,
    height = 4.5
  )
  ...
  p3[[i]] <- ggplot(All_time, aes(x = time / 60, y = O2, color = type)) +
    ...
    geom_line(data = EVZIO_2d[1:timeIndexUL_E2, ], aes(x = time / 60, y = O2)) +
    geom_point(
      data = Generic_1d[timeIndexUL_G1[1], ],
      aes(x = time / 60, y = O2),
      shape = 4,
    ...
    xlim(0, 15)
  ggsave(
    sprintf("results/plot_O2_Individual_%s.png", fo),
    p3[[i]],
    width = 6,
    height = 4.5
  )
  ...
  ggsave(
    sprintf("results/plot_Blood_Individual_%s.png", fo),
    p4[[i]],
    width = 6,
    height = 4.5
  )
...
patchall <- patch1 +
  ...
  patch3 +
  patch4 +
  plot_layout(ncol = 4, nrow = 4, widths = c(1, 1, 1, 1))
patchall2 <- p21 +
  a +
  p22 +
  a +
  ...
  p14 +
  a +
  plot_layout(ncol = 4) #plot_annotation(tag_levels = 'A')
ggsave("results/Manuscript_Figure_7.pdf", patchall2)


p51 <- p5[[1]] +
  ggtitle("Fentanyl 1.63mg \n A") +
  ylab("Plasma Naloxone \n (ng/ml)") +
  theme(plot.title = element_text(size = 10)) +
  theme(
    axis.title.x = element_text(size = 10),
    axis.title.y = element_text(size = 10),
