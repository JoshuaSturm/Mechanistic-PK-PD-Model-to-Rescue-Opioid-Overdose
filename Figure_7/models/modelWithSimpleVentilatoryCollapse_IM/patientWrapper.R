patientWrapper <- function(
  ...
  truepar <- this.par[
    !names(this.par) == "initialdelay" & !names(this.par) == "Dose"
  ]
  ...
  #======================================================================================================================
  Preout1 = fundedewithEvent(
    states = states,
    fulltimes = fulltimes,
    truepar = truepar,
    namesyout = namesyout,
    eventdata
  ) #running the model (without the collapse?)
  P_thrsh_O2 = Preout1[, c("time", "Arterial O2 partial pressure (mm Hg)")][
    Preout1[, c("Arterial O2 partial pressure (mm Hg)")] <= P_a_o2_thrsh,
  ] #store values of time and PaO2 for times where PaO2 < PaO2 threshold so times where collapse should happen due to PaO2
  P_thrsh_CO2 = Preout1[, c("time", "Arterial CO2 partial pressure (mm Hg)")][
    Preout1[, c("Arterial CO2 partial pressure (mm Hg)")] >= P_a_co2_thrsh,
  ] #store values of time and PaCO2 for times where PaCO2 > PaCO2 threshold so times where collapse should happen due to PaCO2

  #	print(P_thrsh_O2)

  if (length(P_thrsh_O2) != 0) {
    #if there are times where PaO2 can cause collapse
    ...
    print("O2 doesn't cross the threshold")
  }
  if (length(P_thrsh_CO2) != 0) {
    #if there are times where PaCO2 can cause collapse
  ...
  if (length(P_thrsh_O2) != 0) {
    #if there are times where PaO2 can cause collapse
    colapse_p = Preout1[
      round(Preout1[, "time"], 1) == round(truepar["CA_delay_o2"], 1),
      "Arterial O2 partial pressure (mm Hg)"
    ] #get PaO2 value at the time collpase is supposed to start
  ...
  try(
    {
      crossinglist <- crossing(patientventi, out[, "time"], threshold)
    },
    silent = T
  )
  ...
  tt1 = t1P[1]
  tt2 = t1P[length(t1P)]
  tt3 = t1P[length(t1P)] - t1P[1]

  if (CA_YN0 == "no" & length(t1P) == 0) {
    #no CA, no Pbo2>20mmhg
    print("dd")
    tt1 = 0
    tt2 = 0
    tt3 = 0
  ...
  if (CA_YN0 == "no" & length(t1P) != 0) {
    #no CA, yes Pbo2<20mmhg
    tt1 = t1P[1]
    tt2 = t1P[length(t1P)]
    tt3 = t1P[length(t1P)] - t1P[1]
  }
  if (CA_YN0 == "yes" & length(t1P) == 0) {
    #yes CA, no Pbo2>20mmhg
    tt1 = 0
    tt2 = 0
    tt3 = 60 * 60
  }
  if (CA_YN0 == "yes" & length(t1P) != 0) {
    tt1 = t1P[1]
    tt2 = t1P[length(t1P)]
    tt3 = 60 * 60 #yes CA, no Pbo2<20mmhg
  }
  ...
  for (naloxone_doseN in 1:max_naloxone_doseN) {
    ...
    dosingoutput <- dosingevents(
      opioid_dose,
      opioid_time = 0,
      naloxone_dose = 2000000,
      naloxone_time = crossingtime + this.par["initialdelay"],
      naloxone_doseN = naloxone_doseN,
      gap = delay
    )
    fulltimes <- dosingoutput[[1]]
    eventdata <- dosingoutput[[2]]
    ...
    if (length(P_thrsh_CO2) != 0) {
      ...
      print("CO2 doesn't cross the threshold")
    }
    CA_YN = "yes"
    if (length(P_thrsh_O2) != 0) {
      #if there are times where PaO2 can cause collapse
    ...
    #--------------------------------------
    rm(out)
    if (length(crossinglist[[2]]) == 1) {
      #only initial crossing; no revoery
    } else {
      crossingtimelist[[naloxone_doseN + 1]] <- crossinglist[[2]][2] -
        crossinglist[[2]][1] +
        this.par["initialdelay"]
    } #if crossing
    rm(crossinglist)
    rm(patientventi)
    rm(eventdata)
