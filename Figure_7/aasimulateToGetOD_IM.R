...
namesyout <- c(
  ...
  "Residual wakefulness drive (l/min)",
  "Chemoreflex drive (l/min)",
  "Blood flow to brain (l/min)",
  "Blood flow to tissue (l/min)",
  "Arterial O2 partial pressure (mm Hg)",
  "Arterial CO2 partial pressure (mm Hg)",
  "Brain O2 partial pressure (mm Hg)",
  "Brain CO2 partial pressure (mm Hg)",
  "Opioid plasma concentration (mg)",
  "Antagonist plasma concentration (ng/ml)",
  "Opioid effect site concentration (pM)",
  "Antagonist effect site concentration (nM)",
  "Opioid bound receptor fraction",
  "Antagonist bound receptor fraction",
  "Total blood flow (l/min)",
  "Elimination Rate (1/s)",
  "K12",
  "K13",
  "K21",
  "K31",
...
parser <- add_option(
  ...
  default = "fentanyl",
  type = "character",
  help = "Opioid used to induce respiratory depression"
)
parser <- add_option(
  parser,
  c("-k", "--usePK"),
  default = "old",
  type = "character",
  help = "whether to use new PK parameters or not"
)
parser <- add_option(
  parser,
  c("-d", "--DThBrainO2"),
  default = "20",
  type = "numeric",
  help = "dangerous brain O2 threshold"
)
parser <- add_option(
  parser,
  c("-e", "--DThArterialO2"),
  default = "15",
  type = "numeric",
  help = "dangerous arterial O2 threshold"
)
parser <- add_option(
  parser,
  c("-t", "--CRT"),
...
dangerousArterialO2Threshold <- strsplit(dangerousArterialO2ThresholdStr, ",")[[
  1
]]
...
populationFolder = sprintf(
  "MCMC_outputs3/final4_EVZIO2_opt_%sParDis_%sDelayDiss_conc%s_PK%s_%s_%s_samplingD_log_Nal0_I",
  consider_Par_dist,
  consider_delay_dist,
  concstr,
  consider_PK_dis,
...
if (opioid == "fentanyl") {
  ...
  parms <- c(
    ...
    A1 = 3.08E-5,
    B1 = 0.004331,
    n = 0.8439, #A1=4.523E-5,B1=.004004,n=.8272,
    A2 = 0.000167,
    B2 = 0.03959,
    n2 = .858,
    V1 = 124700,
...
allpars <- read.table(
  paste0(modelFolder, "fentanyl_pars.txt"),
  header = F,
  as.is = T
)
...
if (usePK == "new") {
  allpatients["kout"] <- 2.109e-4
  allpatients["k12"] <- 1.473e-3
  allpatients["k21"] <- 2.214e-4
  allpatients["k31"] <- 1.932e-5
  allpatients["k13"] <- 4.924e-3
  allpatients["k1"] <- 2.142e-3
  allpatients["VP"] <- 6.655
  #following PK parameters were used when fitting VP3&VP4 (fentanyl bolus injection)
  allpatients["kout"] <- 4.313e-3
  allpatients["k12"] <- 3.171e-3
  allpatients["k21"] <- 1.569e-2
  allpatients["k31"] <- 9.777e-5
  allpatients["k13"] <- 1.92e-3
  allpatients["VP"] <- 7.031
}
...
if (patientType == "naive") {
  ...
  allpatients["P1"] <- 2.5 * 1.47 * 1.15
  allpatients["P3"] <- 0.9 * 1.47 #for 1.3 mg
  #allpatients["k1"]<-0.003
}
#allpatients["k1"]<-10/60
...
if (iman_random_dis == "yes") {
  population0 = read.csv(
    "../parameters/bootpars_Fentanyl_Naloxone_In_Vitro_PK.csv"
  )
...
if (iman_random_dis == "no") {
  ...
  Naloxone_Params <- read.csv(
    "/scratch/john.mann/Iman_inhalation_IV/inhalation/parameters/all2000_EVZIO2.csv"
  )
...
for (opioid_doseidx in 1:length(uniqdose)) {
  ...
  for (ipop in as.numeric(args$patientid)) {
    ...
    if (Plot_yn == "yes") {
      ...
      write.csv(
        ypred1,
        sprintf("%s/%s_%s_ypred1.csv", plotFolder, opioid, concstr)
      )
      write.csv(
        ypred2,
        sprintf("%s/%s_%s_ypred2.csv", plotFolder, opioid, concstr)
      )
    }
  }
  #-----------------------------------------------------------------------------------------------------------------------------
