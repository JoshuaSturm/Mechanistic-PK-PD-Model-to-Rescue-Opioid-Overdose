...
for (iifo1 in alllfolders[caseN]) {
  ...
  write.csv(
    Rescue_Medians,
    sprintf("%s/CA_IM2/Rescue_Times_ligand_%s.csv", foldermainoutput, iifo1)
  )
  ...
  if (
    file.exists(sprintf("%s/%s/All2001.csv", foldermain, folder_of_population))
  ) {
    d2001 = read.csv(
  ...
  CADo_df2$Ndose <- factor(
    CADo_df2$Ndose,
    levels = CADo_df2$Ndose[order(-CADo_df2$rowYesCA)]
  )
  dev.off()

  write.csv(
    CADo_df2,
    sprintf("%s/CA_IM2/CADo1PK%s.csv", foldermainoutput, iifo1)
  )

  CADo_df <- CADo_df[CADo_df$Ndose %in% c(0, 1, 2, "2_S"), ]

  png(
    file = sprintf("%s/barplots_IM2/p0_%s.png", foldermainoutput, iifo1),
    width = 800,
    height = 800
  )
  par(mar = c(6, 8, 2, 2))
