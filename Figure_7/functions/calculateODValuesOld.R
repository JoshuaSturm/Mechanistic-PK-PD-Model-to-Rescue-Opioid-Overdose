calculateODValues <- function(naloxone_doseN) {
  ET <- crossing(
    pp[[1]][[1]][, "Brain O2 partial pressure (mm Hg)"],
    pp[[1]][[1]][, "time"],
    DTh
  )[[2]][1]
  print(paste("ET=", ET / 60, "minutes"))
  print(paste(
    "ventilationThresholdDropTime=",
    ventilationThresholdDropTime / 60,
    "minutes"
  ))
  print(paste(
    "minimum PaO2=",
    min(pp[[1]][[1]][, "Arterial O2 partial pressure (mm Hg)"]),
    "mm Hg"
  ))
  for (Nconditions in 1:naloxonedoseN) {
    pp[[1]][[Nconditions]] <- cbind(
      pp[[1]][[Nconditions]],
      pp[[2]][[Nconditions]]
    )
    if (
      min(pp[[1]][[Nconditions]][, "Brain O2 partial pressure (mm Hg)"]) < DTh
    ) {
      ST <- crossing(
      ...
      if (Nconditions == 1) {
        if (opioid == "fentanyl") {
          #fentanyl
          OPCLT <- crossing(
            pp[[1]][[Nconditions]][, "PlasmaF"] * 1000 / 7.031,
            pp[[1]][[Nconditions]][, "time"],
            3.7
          )[[2]][2]
          OPCMT <- crossing(
          ...
          OPCLT <- crossing(
            pp[[1]][[Nconditions]][, "PlasmaF"] * 1000 / 10.5,
            pp[[1]][[Nconditions]][, "time"],
            0.2
          )[[2]][2]
          OPCMT <- crossing(
