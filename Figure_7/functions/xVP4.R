fVP4 <- function(ind) {
  ...
  if (
    is.na(Vstate[1, "P_a_co2"]) |
      is.nan(Vstate[1, "P_a_co2"]) |
      Vstate[1, "P_a_co2"] > 10000 |
      Vstate[1, "P_a_co2"] < -10000
  ) {
    EVP4 = 10000
  } else {
    for (i in 1:nrow(Vstate)) {
      ...
      Co2_Res = (max(out_slop[, "Venti"]) - min(out_slop[, "Venti"])) /
        (max(out_slop[, "P_a_co2"]) - min(out_slop[, "P_a_co2"])) *
        100 # divide by control slope
    ...
    SlopCO2_exp = read.csv(
      "expdata/CO2_Slope_Response_Rebreathing.csv",
      header = T
    ) /
      100
    #	print(SlopCO2_exp)
    ...
    if (length(All_Co2_Res[, "Co2_Res"]) != length(yexpPSlopCO2)) {
      print("VP4 has a error ! ")
      stopp
    }
    #print("000000000000")
