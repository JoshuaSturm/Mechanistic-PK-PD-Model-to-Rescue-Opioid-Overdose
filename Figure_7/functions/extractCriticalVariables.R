extractCriticalVariables <- function() {
  ...
  for (Nconditions in c(1, 2, 3, 4, 5, 6)) {
    ...
    if (min(ACO2P[, "Total blood flow (l/min)"]) > 1e-1) {
      ...
      id_Brain_20 = which(
        abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 20) ==
          min(abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 20))
      )
      id_O2_30 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 30) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 30))
      )
      id_Brain_25 = which(
        abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 25) ==
          min(abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 25))
      )
      id_O2_65 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 65) ==
      ...
      idSaturation90 = which(
        abs(ACO2P_S[c(idTPS + 1):nrow(ACO2P)] - 90) ==
          min(abs(ACO2P_S[c(idTPS + 1):nrow(ACO2P)] - 90))
      )
      print(nrow(ACO2P))
      print(idTPB)
      #		print(id_Brain_20)
    ...
    PaCO2InflectionPointTime = rbind(
      PaCO2InflectionPointTime,
      PaCO2InflectionPointTime0
    )
  }
  namecsvFile = paste0(
    "opioid=",
  ...
  return(list(
    "PaCO2InflectionPointTime" = PaCO2InflectionPointTime,
    "saturationData" = saturationOutput
  ))
}
