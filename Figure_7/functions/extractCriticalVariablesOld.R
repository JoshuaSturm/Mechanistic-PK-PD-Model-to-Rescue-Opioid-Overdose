extractCriticalVariables <- function(naloxone_doseN) {
  ...
  Turn_p = c()
  for (timeUL in c(30)) {
    for (Nconditions in c(1, 2, 3, 5, 6)) {
      #1:5 corresponds to naloxone doseN 0-4 Added 2 doses as per DGS
      ACO2P = pp[[1]][[Nconditions]][, c(
        "time",
      ...
      id_Pre_55 = which(
        abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 55) ==
          min(abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 55))
      )
      print("CO2_55")
      print(id_Pre_55)
      ...
      print("True_Index")
      id_Pre_50 = which(
        abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 50) ==
          min(abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 50))
      )
      id_Pre_45 = which(
        abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 45) ==
          min(abs(ACO2P_O[c(idTP + 1):nrow(ACO2P)] - 45))
      )
      id_Brain_10 = which(
        abs(ACO2P_B[c(idTPB + 1):571] - 10) ==
          min(abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 10))
      )
      id_Brain_20 = which(
        abs(ACO2P_B[c(idTPB + 1):571] - 20) ==
          min(abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 20))
      )
      id_Brain_25 = which(
        abs(ACO2P_B[c(idTPB + 1):571] - 25) ==
          min(abs(ACO2P_B[c(idTPB + 1):nrow(ACO2P)] - 25))
      )
      id_O2_25 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):571] - 25) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 25))
      )
      print("O2 thresh")
      print(id_O2_25)
      id_O2_30 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):571] - 30) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 30))
      )
      id_O2_50 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):571] - 50) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 50))
      )
      id_O2_65 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):571] - 65) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 65))
      )
      id_O2_75 = which(
        abs(ACO2P_Ox[c(idTPO2 + 1):571] - 75) ==
          min(abs(ACO2P_Ox[c(idTPO2 + 1):nrow(ACO2P)] - 75))
      )
      id_Min_40 = which(
        abs(ACO2P_V[c(idTPV + 1):571] - 2.7) ==
          min(abs(ACO2P_V[c(idTPV + 1):nrow(ACO2P)] - 2.7))
      )
      id_Min_60 = which(
        abs(ACO2P_V[c(idTPV + 1):571] - 4.1) ==
          min(abs(ACO2P_V[c(idTPV + 1):nrow(ACO2P)] - 4.1))
      )
      id_Min_80 = which(
        abs(ACO2P_V[c(idTPV + 1):571] - 5.4) ==
          min(abs(ACO2P_V[c(idTPV + 1):nrow(ACO2P)] - 5.4))
      )
      ...
      Turn_p0 = c(
        ...
        "dose" = args$conc,
        "Ndose" = Ndosen,
        "firsNinj" = ventilationThresholdDropTime,
        "RT" = -(ventilationThresholdDropTime - ACO2P[idTP, "time"]),
        ACO2P[
          idTP + id_Pre_55,
          c(
            "time",
