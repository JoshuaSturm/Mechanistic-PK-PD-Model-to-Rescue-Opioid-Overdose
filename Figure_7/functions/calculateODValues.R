calculateODValues <- function() {
  for (Nconditions in c(1)) {
    ...
    #		print((min(pp[[1]][[Nconditions]][,"Total blood flow (l/min)"])/pp[[1]][[Nconditions]][1,"Total blood flow (l/min)"])*100)
    if (
      min(pp[[1]][[Nconditions]][, "Total blood flow (l/min)"]) <= CABloodFlow
    ) {
      ETBloodFlow <- crossing(
        pp[[1]][[Nconditions]][, "Blood flow to brain (l/min)"] +
          pp[[1]][[Nconditions]][, "Blood flow to tissue (l/min)"],
      ...
      if (Nconditions == 1) {
        if (opioid == "fentanyl") {
          #fentanyl
          OPCLT <- crossing(
            pp[[1]][[1]][, "PlasmaF"] * 1000 / pp[[1]][[1]][, "Fentanyl VoD"],
            pp[[1]][[1]][, "time"],
            3.7
          )[[2]][2]
          OPCMT <- crossing(
