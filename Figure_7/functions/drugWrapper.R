drugWrapper <- function(
  ...
  recovery_mat <- matrix(
    0,
    nrow = dim(allpatients)[1],
    ncol = max_naloxone_doseN
  ) #4 doses of naloxone
  ...
  for (patientidx in 1:dim(allpatients)[1]) {
    ...
    write.csv(
      Output,
      paste0("Fentanyl_NWC_parameters/CARMolarMass_Last_wc_95.csv")
    )
    ...
    for (naloxone_doseN in 1:6) {
      ...
      if (length(crossinglist[[2]]) == 1) {
        ...
        recovery_mat[patientidx, naloxone_doseN] <- crossinglist[[2]][2] -
          crossinglist[[2]][1]
      } #if crossing
      rm(crossinglist)
      rm(patientventi)
