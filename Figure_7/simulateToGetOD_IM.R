...
parser <- add_option(
  ...
  default = "EVZIO",
  type = "character",
  help = "Type of Naloxone IM formulation: either EVZIO or Generic"
)
...
populationFolder = sprintf(
  "outputs/Review_naloxone_formulation_%s_conc_%s_ligand_%s_patient_%s",
  formulation,
  concstr,
  opioid,
  patientType
...
if (random_dis == "no") {
  # PK populations for opioids (fentanyl or carfentanil)
  population0 = c()
