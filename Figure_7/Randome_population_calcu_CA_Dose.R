...
parser <- add_option(
  ...
  default = "1",
  type = "numeric",
  help = "which naloxone opioid combination to use"
)
...
#ddddddd
alllfolders = c(
  "Review_naloxone_formulation_EVZIO_conc_1.625_ligand_fentanyl_patient_chronic",
...
for (iran in 1:2500) {
  ...
  for (iifo1 in alllfolders[caseN]) {
    ...
    for (i in isam) {
      if (
        file.exists(sprintf(
          "%s/%s/All%s.csv",
          foldermain,
          folder_of_population,
          i
        ))
      ) {
        d0 = read.csv(
          sprintf("%s/%s/All%s.csv", foldermain, folder_of_population, i),
          stringsAsFactors = F
        )
    ...
    system(paste0(
      "mkdir -p ",
      sprintf("%s/barplots_Errorbar", foldermainoutput)
    ))
    ...
    if (
      ...
      d2001 = read.csv(
        sprintf("%s/%s/All2001.csv", foldermain, folder_of_population),
        stringsAsFactors = F
      )
    } else {
      d2001 = c(rep("NAAA", 2))
    ...
    if (iran == 1) {
      ...
      write.csv(
        Rescue_Medians,
        sprintf(
          "%s/CA_IM/%s_Rescue_Times_ligand_2_%s.csv",
          foldermainoutput,
          iran,
          iifo1
        )
      ...
      write.csv(
        CADo_df2,
        sprintf("%s/CA_IM/%s_CADo1PK%s.csv", foldermainoutput, iran, iifo1)
      )

      png(
        file = sprintf(
          "%s/barplots_IM/%s_p0_%s.png",
          foldermainoutput,
          iran,
          iifo1
        ),
      ...
      title(
        ylab = "Cardiac Arrest Percentage",
        cex.lab = 3.5,
        line = 4.5,
        col.lab = "darkblue"
      )
      title(xlab = "Number of Naloxone doses", cex.lab = 3.5, line = 4.5)
      dev.off()
    }
  }
}
...
for (indos in unlist(unique(CADo_dfmean["Ndose"]))) {
  ...
  write.csv(
    CADo_dfmean,
    sprintf("%s/CA_IM/All_CADo1PK%s.csv", foldermainoutput, iifo1)
  )
}
CADo_dfmean$txtt = sprintf(
  "%s (%s-%s)",
  round(CADo_dfmean$rowYesCA, 0),
  round(CADo_dfmean$emin, 0),
  round(CADo_dfmean$emax),
  0
)
...
p1 <- p1 +
  ggtitle(sprintf("%s", labellsi)) +
  ylim(0, 100) +
  theme(legend.position = "none") +
  theme(plot.title = element_text(size = 12, face = "bold")) +
  labs(x = "Naloxone dose (mg)", y = "Recovered % of virtual subjects") +
  theme_bw() +
  theme(
...
ggsave(
  sprintf("%s/barplots_Errorbar/%s_dose.png", foldermainoutput, iifo1),
  p1,
  width = 3.5,
  height = 3.8
)
