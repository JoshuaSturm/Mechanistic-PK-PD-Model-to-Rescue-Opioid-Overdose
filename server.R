server <- function(input, output, session) {
	...
	route_list <- reactive({
				route_data %>% data.frame %>% filter(contaminant == selected_contaminant) %>% select(route)
			})
	# dataframe with all model for selected route
	model_list <- reactive({
				model_data %>% data.frame %>% filter(contaminant == selected_contaminant, route == selected_route) %>% select(model)
			})
	# dataframe with all distribution options for selected route
	distribution_list <- reactive({
				distribution_data %>% data.frame %>% filter(contaminant == selected_contaminant, route == selected_route, model == selected_model) %>% select(distribution)
			})
	
	# full dataframe for all parameters for selected model
	par_list <- reactive({
				par_data %>% data.frame %>% filter(contaminant == selected_contaminant, route == selected_route, model==selected_model)
			})
	...
	output$select_distribution <- renderUI({
				req(selected_route)
				tagList(
						selectInput("distribution", "Parameter distribution for dermal and inhalation absorption:",
	...
	observeEvent({
				selected_contaminant
				selected_route},
			{
				
				settings <- updated_par_data$settings %>% filter(contaminant == selected_contaminant, model==selected_model, route == selected_route)
	...
	observeEvent({
				...
				selected_route},
			{
				...
				if (selected_model == "Extrapolation from Lifetime Exposure to Benzene in the Air (EPA IRIS 2000)"){
					risk <- EPA_equation(selected_route, df=population$pops)
				}
	...
	group4_out <- grouped_par_dists_Server("group4", parent=session$input, group_idx=4)
	group5_out <- grouped_par_dists_Server("group5", parent=session$input, group_idx=5)
	
	# update the settings and sampled population based on group 1 changes
	observe({
				req(group1_out(), selected_contaminant, selected_route)
				if (!is.null(group1_out()$settings) & !is.null(group1_out()$par)){

					
					updated_par_data$settings[which(updated_par_data$settings[,"contaminant"]==selected_contaminant &
									updated_par_data$settings[,"model"] == selected_model &
									updated_par_data$settings[,"route"]==selected_route &
	...
	observe({
				req(group2_out(), selected_contaminant, selected_route)
	...
	observe({
				req(group3_out(), selected_contaminant, selected_route)
	...
	# update the settings and sampled population based on group 4 changes
	observe({
				req(group4_out(), selected_contaminant, selected_route)
				if (!is.null(group4_out()$settings) & !is.null(group4_out()$par)){
					
					
					updated_par_data$settings[which(updated_par_data$settings[,"contaminant"]==selected_contaminant &
											updated_par_data$settings[,"model"] == selected_model &
											updated_par_data$settings[,"route"]==selected_route &
											updated_par_data$settings[,"par"] == c(group4_out()$par)),] <- group4_out()$settings
					
				}
				if (!is.null(group4_out()$pop) & !is.null(group4_out()$par)){
					population$pops[,which(names(population$pops) == group4_out()$par)] <- group4_out()$pop
				}
			})
	
	# update the settings and sampled population based on group 5 changes
	observe({
				req(group5_out(), selected_contaminant, selected_route)
				if (!is.null(group5_out()$settings) & !is.null(group5_out()$par)){
					
					
					updated_par_data$settings[which(updated_par_data$settings[,"contaminant"]==selected_contaminant &
											updated_par_data$settings[,"model"] == selected_model &
											updated_par_data$settings[,"route"]==selected_route &
											updated_par_data$settings[,"par"] == c(group5_out()$par)),] <- group5_out()$settings
					
				}
				if (!is.null(group5_out()$pop) & !is.null(group5_out()$par)){
					population$pops[,which(names(population$pops) == group5_out()$par)] <- group5_out()$pop
				}
			})

	risk_probs <- reactive({
				req(selected_route, group1_out(),group2_out(),group3_out(), group4_out(), group5_out(), risk_prob$default)
				# calculate risk (uses functions/EPA_equation.R)
				if (selected_model == "Extrapolation from Lifetime Exposure to Benzene in the Air (EPA IRIS 2000)"){
					risk <- EPA_equation(selected_route, df=population$pops)
				}
	...
	output$Totals<-renderPlot({
				req(selected_route, group1_out(),group2_out(),group3_out(), group4_out(), group5_out(), risk_prob$default, risk_probs())
...
#						legend.position = "none",
	...
	output$settings <- renderUI({
				req(selected_contaminant, selected_route, input$distribution, group1_out(),group2_out(),group3_out(), group4_out(), group5_out())
				...
				warning_tag <- "<td style='background-color:#FA5F55'>" # lighter and easier to read
#				warning_tag <- "<td style='background-color:#D60036'>" # changed to match FDA color scheme https://fda.sharepoint.com/:u:/r/sites/OC-Intranet-OC-OEA-VisualIdentityProgram/SitePages/Color.aspx?csf=1&web=1&e=Qi6yVT
				
				alt_warning <- "<td style='color:#fcba03;border: 0px solid black;background-color:transparent;'>Warning: selected values are from an alternative distribution (see data sources)</td>"
				alt_warning_tag <- "<td style='background-color:#fcba03'>" # I think I prefer this color over the FDA gold
#				alt_warning_tag <- "<td style='background-color:#E5B611'>" # changed to match FDA color scheme https://fda.sharepoint.com/:u:/r/sites/OC-Intranet-OC-OEA-VisualIdentityProgram/SitePages/Color.aspx?csf=1&web=1&e=Qi6yVT
				
				cleaned_distribution <- gsub("\\s", "_", input$distribution) 
				...
				for (this_par in par_list()$par){
					...
					parameter_settings <- updated_par_data$settings %>% filter(contaminant == selected_contaminant, model == selected_model, route == selected_route, par == this_par)
					...
					default_settings <- default_distribution_settings() %>% filter(contaminant == selected_contaminant, model == selected_model, route == selected_route, par == this_par)
					
					alt_default_settings <- default_settings[, grepl(cleaned_distribution, colnames(default_settings), fixed = TRUE)]
					colnames(alt_default_settings) <- gsub(paste0(cleaned_distribution,"."), "", colnames(alt_default_settings))
					colnames(alt_default_settings)[colnames(alt_default_settings) != "dist_to_use"] <- paste0(alt_default_settings[,"dist_to_use"],".",colnames(alt_default_settings)[colnames(alt_default_settings) != "dist_to_use"])
					warnings_text <- ""
					front_tag <- "<td>"
					...
					if(parameter_settings$dist_to_use != default_settings$dist_to_use){
						...
						filtered_default <- default_settings %>% select(!c(..JSON, document.id, contaminant.idx, contaminant, model.idx, model, 
						...
						filtered_settings <- parameter_settings %>% select(!c(..JSON, document.id, contaminant.idx, contaminant, model.idx, model, 
										route.idx, route, group.idx, groups, par.idx, par, unit, suffix, 
										source, source_image_path, dist_to_use,
										Experimentally_derived_distributions.dist_to_use,
										Experimentally_derived_distributions.mean.init,
					...
					# if there are alternative defaults
					if (all(!is.na(alt_default_settings)) & input$distribution != default_dist){
						if(identical(alt_default_settings, parameter_settings[,which(colnames(parameter_settings) %in% colnames(alt_default_settings))])){
							warnings_text <- alt_warning
							front_tag <- alt_warning_tag
						}
	...
	output$dose_text <- renderUI({
				req(selected_contaminant, selected_route, group1_out(),group2_out(),group3_out(), group4_out(), group5_out())
				product_absorption <- population$pops[,"Benzene Absorption (dermal)"]/100
				product_absorption[which(product_absorption > 1)] <- 1		
				product_absorption[which(product_absorption < 0)] <- 0
				
				inhaled_absorption <- population$pops[,"Benzene Absorption (inhaled)"]/100
				inhaled_absorption[which(inhaled_absorption > 1)] <- 1		
				inhaled_absorption[which(inhaled_absorption < 0)] <- 0
				
				inhaled_fraction <- population$pops[,"Evaporated Benzene"]/100
				inhaled_fraction[which(inhaled_fraction > 1)] <- 1		
				inhaled_fraction[which(inhaled_fraction < 0)] <- 0
				
				absorbed_fraction <- population$pops[,"Benzene Available for Dermal Absorption"]/100
				...
				breathing_rate <- population$pops[,"Minute Ventilation"]
				t1 <- population$pops[,"Exposure time (near-field)"]
				V1 <- population$pops[,"Distribution volume (near-field)"]
				t2 <- population$pops[,"Exposure time (far-field)"]
				V2 <- population$pops[,"Distribution volume (far-field)"]
				...
				dose <- product_conc*((dermal_dose+inhaled_dose)*use_per_day)
				dose_quant <- quantile(dose, c(0.025, 0.25, 0.5, 0.75, 0.975))
				
				text <- paste0("<h5>Lifetime Average Daily Dose:</h5>The median lifetime average daily dose of ", selected_contaminant," for the product is ", signif(dose_quant[["50%"]],3)," μg/day (95%CI, ",signif(dose_quant[["2.5%"]],3)," to ",signif(dose_quant[["97.5%"]],3)," μg/day)")
				HTML(text)
			})
	...
	output$par_data <- renderUI({
				...
				pars_to_choose <- list(list(par_list()[par_list()$group.idx==1, "par"]),
						...
						list(par_list()[par_list()$group.idx==3, "par"]),
						c(par_list()[par_list()$group.idx==4, "par"]),
						c(par_list()[par_list()$group.idx==5, "par"]))
